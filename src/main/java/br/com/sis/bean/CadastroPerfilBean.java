...
public class CadastroPerfilBean implements Serializable {
	...
	public void inicializar() {
		...
		paginasPerfil.clear();
		perfil.getFuncionalidades().sort(null);
		for (Funcionalidade p : perfil.getFuncionalidades()) {
	...
	public void salvar() {
		...
		for (String s : paginasPerfil) {
			...
			perfil.getFuncionalidades().add(p);
		}
	...
	public boolean isPermissaoNovo() {
		return seguranca.isPerfilInserir();
	}
	
	public boolean isPermissaoSalvar() {
		return (this.isEditando() && this.seguranca.isPerfilEditar()) || (!this.isEditando() && this.seguranca.isPerfilInserir());
	}	
}
